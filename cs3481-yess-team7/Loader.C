...
void Loader::loadLine(std::string line)
{
   ...
   //Also, use the convert method for each by
   ...
   {
        dataByte = convert(line, i, 2);
        mem -> putByte(dataByte, addr + count, error);
        count++;    
   }
}
...
{
   //Hint: you need something to convert a string to an int such as stoul or strtol
   std::string str = line.substr(start, len);
   uint32_t convertedStr = stoul(str, NULL, 16);
   return convertedStr;
}
...
bool Loader::hasErrors(std::string line)
{
   ...
   if (!hasData(line)) return !(isSpaces(line, DATABEGIN, COMMENT - 1));
   ...
   //   to lastAddress
    int32_t currAddress = convert(line, ADDRBEGIN, ADDREND);
    if (currAddress < (lastAddress)) return true;
   ...
   //         and addr returned by convert
    if ((currAddress + numDBytes) > (MEMSIZE)) 
        return true;
    lastAddress = currAddress + numDBytes;
...
bool Loader::errorData(std::string line, int32_t & numDBytes)
{
    ...
    for (; i < (COMMENT - 1); i++) {
        if (line[i] == ' ') break;
        if (!(isxdigit(line[i])) && (line[i] != '0')) return true;
        count++;
    }
...
bool Loader::errorAddr(std::string line)
{
    ...
    if (line[ADDRBEGIN - 1] != 'x') return true;
    if (line[ADDREND + 1] != ':') return true;
    if (line[ADDREND + 2] != ' ') return true;
 ...
 *         false - otherwise
 */
bool Loader::goodFile(std::string filename) {
    if(filename.length() >=4) {
        if(filename.find(".yo") != std::string::npos) {
            return true;
        }
    }
    return false;
